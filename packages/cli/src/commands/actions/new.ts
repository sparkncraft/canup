...
// number. The character classes already bound length to 2..64.
const ACTION_SLUG_PATTERN = /^[a-z0-9][a-z0-9-]{0,62}[a-z0-9]$/;

function isValidActionSlug(name: string): boolean {
  return (
    name.length >= ACTION_SLUG_MIN_LENGTH &&
    name.length <= ACTION_SLUG_MAX_LENGTH &&
    ACTION_SLUG_PATTERN.test(name)
  );
}
